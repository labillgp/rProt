graph_myheatplot <- function (dataset, dend = c("both", "row", "column", "none"),
          ...
          zlim = c(-3, 3), scaleKey = TRUE, Colv = NULL, Rowv = NULL, ...)
{
  print(labels(Colv))
  library(gplots)
  ...
  distSpearman <- function(x, use = "pairwise.complete.obs") {
    as.dist(1-cor(x, use = use, method='spearman'))
  }
  distCosine <- function(M) {
    as.dist(cosine(M))  ## From lsa library
  }
  ...
  if (is.null(distfun))
    distf = distEisen
  else if (distfun=='spearman') {
    print('Using Spearman correlation distance')
    distf = distSpearman
  } else if (distfun=='cosine') {
